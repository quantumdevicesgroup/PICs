...
for i in range(20):
	...
	PIC.x, PIC.y = 3*mm+1*um,4.5*mm+i*50*um
	...
	rs = 10*um # rectangle spacing
	rw = (i+1)*0.25*um # rectangle width
	for j in range(11):
		x1 = xc + (j-5)*rs
		x2 = xc + rw + (j-5)*rs
		y1 = yc + (j-5)*rs
		y2 = yc + rw + (j-5)*rs
		PIC.design.add(gdspy.Rectangle((x1,y1),(x2,y2),layer=11,datatype=0))
...
for j in range(2):
	for i in range(3):
		...
		PIC.tp(50*um,300*nm,50*nm)
		#xc,yc = x1+(x2-x1)/2, y1+(y2-y1)/2 + dy/2
		#PIC.design.add(gdspy.Rectangle((xc,yc),(xc+10*um,yc+dy),layer=11,datatype=0))
