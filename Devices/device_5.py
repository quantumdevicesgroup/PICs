...
for i in range(11):
	for j in range(3):
		...
		x,y = chip.etch_tp((3*mm+(j-1)*10*um+(i-5)*2*um,4*mm+i*125*um+j*25*um),50*um,130*nm,300*nm)
		x,y = chip.etch_wg([(x,y),(3.21*mm + 370*nm*34,y)])
...
for i in range(3):
	...
	xb,yb = chip.etch_tp((3*mm+(i-1)*5*um,5.5*mm+50*um*i),50*um,130*nm,300*nm)
	x,y = xb,yb
...
for i in range(3):
	#x,y = chip.etch_wg([(1.95*mm+(i-1)*5*um,5.7*mm+50*um*i),(2*mm+(i-1)*5*um,5.7*mm+50*um*i)],130*nm)
	xb,yb = chip.etch_tp((3*mm+(i-1)*5*um,5.7*mm+50*um*i),50*um,130*nm,300*nm)
	x,y = xb,yb
