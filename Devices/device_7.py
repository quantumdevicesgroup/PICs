...
chip.deep_etch(3*mm)
chip.etch_markers([	(3.25*mm,6*mm),(6.75*mm,6*mm),(6.75*mm,3.75*mm)],200*um, 10*um)
...
for i in range(11):
	for j in range(3):
		...
		x,y = chip.etch_tp((3*mm+(j-1)*5*um+(i-5)*1*um,4*mm+i*125*um+j*25*um),50*um,130*nm,300*nm)
		x,y = chip.etch_wg([(x,y),(3.51*mm + 370*nm*34,y)])
...
for i in range(3):
	...
	xb,yb = chip.etch_tp((3*mm+2*um+(i-1)*1*um,5.5*mm+50*um*i),50*um,130*nm,300*nm)
	x,y = xb,yb
	dy = 300*nm*(0.5+0.5+1) + 2*150*nm + 5*um*2
	x,y = chip.etch_wg(([x,y],[x+400*um,y]))
...
for i in range(3):
	...
	xb,yb = chip.etch_tp((3*mm+2*um+(i-1)*1*um,5.7*mm+50*um*i),50*um,130*nm,300*nm)
	x,y = xb,yb
...
for i in range(11):
	for j in range(3):
		...
		x,y = chip.etch_tp((10*mm-3*mm-(j-1)*5*um-(i-5)*1*um - 50*um ,4*mm+i*125*um+j*25*um),50*um,300*nm,130*nm)
		#x,y = chip.etch_tp((x-50*um,y),50*um,300*nm,130*nm)
		x,y = chip.etch_wg([(x-50*um,y),(10*mm-3.51*mm - 370*nm*34,y)])
...
for i in range(3):
	...
	xb,yb = chip.etch_tp((10*mm-3*mm-2*um-(i-1)*1*um-50*um,5.5*mm+50*um*i),50*um,300*nm,130*nm)
...
for i in range(3):
	...
	xb,yb = chip.etch_tp((10*mm-3*mm-2*um-(i-1)*1*um-50*um,5.7*mm+50*um*i),50*um,300*nm,130*nm)
	#xb,yb = chip.etch_tp((x-50*um,y),50*um,300*nm,130*nm)
	x,y = xb-50*um,yb
	dy = 300*nm*(0.5+0.5+1) + 2*250*nm + 5*um*2
	x,y = chip.etch_wg(([x,y],[x-400*um,y]))
